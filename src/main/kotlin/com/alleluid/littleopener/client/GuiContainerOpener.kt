...
class GuiContainerOpener(val container: ContainerOpener, val posTE: BlockPos) : GuiContainer(container) {
    val X_FIELD = 0
    val Y_FIELD = 1
    val Z_FIELD = 2
    ...
    override fun initGui() {
        super.initGui()
//        val targetPos = (Minecraft.getMinecraft().world.getTileEntity(posTE) as TileOpener).targetPos
        val targetPos = container.tileOpener.targetPos
...
//        PacketHandler.INSTANCE.sendToServer(CoordsMessage(posTE, posLT))
        container.targetPos = posLT
        super.onGuiClosed()
    }
